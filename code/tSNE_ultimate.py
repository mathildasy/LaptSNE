...
def gk_grad(Y,num_eigen, sigmas, UPDATE_SIGMA = True, min_k = False, poly = 0, const = 0):
    '''
...
def poly_grad(Y, num_eigen,sigmas, UPDATE_SIGMA, min_k, poly,const):
    global lam_list
    print(' ------------Start poly_grad ---------------')
    Kval = np.power((Y @ Y.T + const), poly)
    n, d = Y.shape[0], Y.shape[1]
    print(n - num_eigen)
    D = np.diag(Kval.sum(axis=0))
    if min_k:
        lam, eig_V = eigh(power_diag(D, -0.5) @ Kval @ power_diag(D, -0.5),
                          subset_by_index=[n - num_eigen - 1, n - num_eigen])
        eig_V_kp1 = eig_V[:, 0]
        eig_V = eig_V[:, 1]
    else:
        lam, eig_V = eigh(power_diag(D, -0.5) @ Kval @ power_diag(D, -0.5), subset_by_index=[n - num_eigen - 1, n - 1])
    ...
    grad_LK = (U0 + U1 + power_diag(D, -1)) * (eig_V @ eig_V.T)
    grad_LK1 = (U0 + U1 + power_diag(D, -1)) * (eig_V_kp1 @ eig_V_kp1.T)
    grad_Y = 2 * poly * (grad_LK * (np.power((Y @ Y.T + const), (poly - 1)))) @ Y
...
def tsne_grad(grad, P, Q, Y,  inv_distances, beta, beta_2, poly, const, num_eigen,sigmas, UPDATE_SIGMA = True, min_k = False):
    """
    ...
    grad_sigma = 0
    if beta != 0:
        grad_Y2, grad_Y3, grad_sigma = grad(Y, num_eigen,sigmas, UPDATE_SIGMA, min_k = min_k, poly = poly, const = const)
...
def estimate_sne(X, y, P, num_iters, q_fn, grad, learning_rate1, learning_rate2, momentum, beta, beta_2, poly, const, num_eigen, plot,
    ...
    for i in tqdm(range(num_iters)):
        ...
        grads_Y, grad_sigma = tsne_grad(grad, P2, Q, Y_update, distances, beta2, beta_3, poly, const, num_eigen, sigmas, UPDATE_SIGMA = True, min_k = min_k)
        # Update Y
